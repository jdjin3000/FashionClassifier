...
def seperateImages():
    ...
    for element in lists:
        ...
        if not os.path.exists(os.path.join(os.path.join(os.path.join(os.path.join(classify_path, element[2]), element[1])), element[0].split('/')[1])):
            os.mkdir(os.path.join(os.path.join(os.path.join(os.path.join(classify_path, element[2]), element[1])), element[0].split('/')[1]))
        
        shutil.copy(os.path.join('.', element[0]), os.path.join(os.path.join(os.path.join(classify_path, element[2]), element[1]), element[0].split('/')[1]))
    ...
    for element in lists:
        ...
        if not os.path.exists(os.path.join(os.path.join(os.path.join(os.path.join(bbox_path, element[2]), element[1])), element[0].split('/')[1])):
            os.mkdir(os.path.join(os.path.join(os.path.join(os.path.join(bbox_path, element[2]), element[1])), element[0].split('/')[1]))

        img = cv2.imread(os.path.join('.', element[0]))
        cropped_img = img[element[-1][1]:element[-1][3], element[-1][0]:element[-1][2]]
        cv2.imwrite(os.path.join(os.path.join(os.path.join(os.path.join(bbox_path, element[2]), element[1]), element[0].split('/')[1]), element[0].split('/')[2]), cropped_img)

    print("2. Cropping Image has done Successfully!")
